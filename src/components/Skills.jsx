    ...
    <section className="py-20 px-6 md:px-20 bg-gradient-to-br from-gray-900 via-black to-gray-800 text-slate-300">
      ...
      <div className="grid md:grid-cols-2 gap-8 max-w-[1270px] mx-auto">
        {skills.map((category, index) => (
          <div
            key={index}
            className="bg-gray-800/50 p-6 rounded-xl shadow-md"
          >
            ...
            <div className="flex flex-wrap gap-4">
              {category.items.map((item, i) => (
                <div
                  key={i}
                  className="w-20 h-20 bg-gray-900 border border-gray-700 rounded-lg flex flex-col items-center justify-center transition-transform hover:scale-110 hover:shadow-lg"
                >
                  <img
                    src={item.icon}
                    alt={item.name}
